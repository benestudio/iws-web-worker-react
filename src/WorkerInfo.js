...
workerInstance.onmessage = function(e) {
  ...
  if (type === 'csvFormat') {
    ...
    downloadBlobToFile(new Blob([data],{ type:  contentTypeCSV}),`github_${uniqTime}.csv`);
  const clickEnd = new Date().getTime();
  console.log('main:: The whole process took: ' + (clickEnd - startTime) + ' ms');
...
function WorkerInfo({workerMode}) {
    ...
    if (status === 'resolved') {
      return getSnippet(<pre>Time taken for the last operation is: {performance}ms.</pre>)
    }
...
function blockingExport(data) {
    ...
    const clickEnd = new Date().getTime();
    console.log('main:: The whole process took: ' + (clickEnd - clickStart) + ' ms');
...
/* This method will call the worker with a particular type that maps to a callback to format the csv
   ...
   * @return {Undefined}
   */
const getCSV = (data,clickStart,transferMode = 'csvFormat') => {
...
function nonBlockingExport(data,transferable=false) {
    ...
    getCSV(data,clickStart,"blobber");
...
function nonBlockingTransferableExport(data){
  ...
  getCSV(data,clickStart,"blobber-transfer");
  ...
  const getBlob = (csvFile, startTime) => {
    ...
    console.log('main:: blob workerMaker takes ' + blobFormatTime + ' ms to start');
  }
